...
/**
 * Proxy utility to forward requests to admin-service
 * @param req Express request object
 * @param res Express response object
 * @param targetPath Target path on admin-service (e.g., "/api/v1/admin/patients")
 * @param method HTTP method (defaults to req.method)
 */
export async function proxyToAdminService(
  ...
  const startTime = Date.now();
  const httpMethod = method || req.method;
  const baseUrl = envConf.ADMIN_SERVICE_URL;
  
  // Get provider ID from req.user (set by isProvider middleware)
  const user = (req as any).user as { id: string; userType: string } | undefined;
  ...
  // Add providerId to query params if available (admin-service may use this)
  if (providerId) {
  ...
  try {
    ...
    let requestBody = req.body;
    if (["POST", "PUT", "PATCH"].includes(httpMethod) && requestBody) {
      // Add providerId to body if not already present
      if (providerId && typeof requestBody === "object" && !requestBody.providerId) {
        requestBody = { ...requestBody, providerId };
      }
      if (Object.keys(requestBody).length > 0) {
        options.body = JSON.stringify(requestBody);
      }
    ...
    // Make request to admin-service using fetch (Node.js 18+)
    const response = await fetch(url.toString(), options);
