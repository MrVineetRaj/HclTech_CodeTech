...
class Controller {
  ...
  public async registerPatient(
    ...
    try {
      const { healthProviderId, fullname, email, password, phone, address, city, state, pincode, country } = req.body;

      // Validate required fields
      if (!healthProviderId || !fullname || !email || !password || !phone || !address || !city || !state || !pincode || !country) {
        throw new ErrorResponse({
          statusCode: 400,
          success: false,
      ...
      // Check if health provider exists
      ...
      delete (patientData as any).password;
  ...
  public async loginPatient(
    ...
    try {
      ...
      // Set user in session (passport will handle this)
      const authReq = req as AuthenticatedRequest;
      authReq.user = {
  ...
  public async logoutPatient(
    ...
    try {
      // Clear user session - passport handles this via req.logout if available
      ...
      // If using express-session, destroy the session
